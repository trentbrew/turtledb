...
// --------- Helpers & Styling ---------
const mockEmbed = (_v: string) => Promise.resolve(Array(5).fill(0));
  ...
  const int = setInterval(() => {
    Deno.stdout.writeSync(
      new TextEncoder().encode(
        `\r${colors.cyan}${
          SPINNER[spinIdx % SPINNER.length]
        }${colors.reset} ${msg}`,
      ),
    );
    spinIdx++;
...
function colorJSON(v: unknown): string {
...
async function saveGraph(name: string, g: GraphCore) {
...
async function exec<T>(msg: string, task: Promise<T>): Promise<T | null> {
...
if (USE_MOCK_EMBED) {
  console.log(
    `${colors.gray}Using mock embeddings (--mock-embed flag)${colors.reset}`,
  );
  ...
  try {
    ...
    console.warn(
      `${colors.yellow}⚠ Falling back to mock embeddings: ${e.message}${colors.reset}`,
    );
    graph = new GraphCore(schema, { embeddingFn: mockEmbed });
  }
}
...
function help() {
  ...
  console.log(
    `  add node <type> <json>     - Example: add node person {"name":"alice"}`,
  );
  console.log(`  add edge <type> <srcId> <tgtId> <json>`);
  console.log(
    `  list [types]              - List nodes/edges or available types`,
  );
  console.log(`  schema [node|edge] [type] - View detailed schema information`);
  console.log(
    `  inspect <nodeId>          - View detailed info about a specific node`,
  );
...
async function cmd(line: string) {
  ...
  switch (cmd) {
      ...
      if (a[0] === "node") {
        ...
        if (jsonParts.length > 0) {
          try {
            ...
            console.log(
              `${colors.red}Invalid JSON:${colors.reset} ${e.message}`,
            );
            console.log('Example: add node person {"name":"alice"}');
            break;
          }
        ...
        const e = await exec(
          "adding edge",
          Promise.resolve(graph.createEdge(type, src, tgt, data)),
        );
        if (e) console.log(colorJSON(e));
        await persist();
      }
      break;
    case "schema":
      {
        ...
        if (!category) {
          ...
          console.log(
            `${colors.bright}${colors.blue}Schema Overview${colors.reset}`,
          );
          ...
          for (const [name, nodeType] of Object.entries(schema.node_types)) {
            ...
            if (nodeType.synonyms?.length) {
              console.log(
                `    ${colors.gray}Synonyms: ${
                  nodeType.synonyms.join(", ")
                }${colors.reset}`,
              );
            }
          }
          ...
          for (const [name, edgeType] of Object.entries(schema.edge_types)) {
            console.log(
              `  ${colors.green}${name}${colors.reset}: ${
                edgeType.description || "No description"
              }`,
            );
            console.log(
              `    ${colors.gray}${edgeType.source.node_type} → ${edgeType.target.node_type}${colors.reset}`,
            );
          }
        } else if (category === "node") {
          if (!typeName) {
            console.log(
              `${colors.bright}${colors.blue}Node Types:${colors.reset}`,
            );
            for (const [name, nodeType] of Object.entries(schema.node_types)) {
              console.log(`\n${colors.green}${name}${colors.reset}`);
              console.log(
                `  Description: ${nodeType.description || "No description"}`,
              );
              if (nodeType.synonyms?.length) {
                console.log(
                  `  Synonyms: ${colors.gray}${
                    nodeType.synonyms.join(", ")
                  }${colors.reset}`,
                );
              }
            ...
            if (!nodeType) {
              console.log(
                `${colors.red}Node type '${typeName}' not found${colors.reset}`,
              );
              console.log(
                `Available types: ${Object.keys(schema.node_types).join(", ")}`,
              );
            } else {
              console.log(
                `${colors.bright}${colors.blue}Node Type: ${colors.green}${typeName}${colors.reset}`,
              );
              console.log(
                `Description: ${nodeType.description || "No description"}`,
              );
          ...
          if (!typeName) {
            console.log(
              `${colors.bright}${colors.blue}Edge Types:${colors.reset}`,
            );
            for (const [name, edgeType] of Object.entries(schema.edge_types)) {
              console.log(`\n${colors.green}${name}${colors.reset}`);
              console.log(
                `  Description: ${edgeType.description || "No description"}`,
              );
              console.log(
                `  Connection: ${colors.cyan}${edgeType.source.node_type}${colors.reset} → ${colors.cyan}${edgeType.target.node_type}${colors.reset}`,
              );
              console.log(
                `  Source: ${
                  edgeType.source.multiple ? "multiple" : "single"
                }, ${edgeType.source.required ? "required" : "optional"}`,
              );
              console.log(
                `  Target: ${
                  edgeType.target.multiple ? "multiple" : "single"
                }, ${edgeType.target.required ? "required" : "optional"}`,
              );
              console.log(`  Data Schema:`);
            ...
            if (!edgeType) {
              console.log(
                `${colors.red}Edge type '${typeName}' not found${colors.reset}`,
              );
              console.log(
                `Available types: ${Object.keys(schema.edge_types).join(", ")}`,
              );
            } else {
              console.log(
                `${colors.bright}${colors.blue}Edge Type: ${colors.green}${typeName}${colors.reset}`,
              );
              console.log(
                `Description: ${edgeType.description || "No description"}`,
              );
              ...
              console.log(
                `Source: ${edgeType.source.multiple ? "multiple" : "single"}, ${
                  edgeType.source.required ? "required" : "optional"
                }`,
              );
              console.log(
                `Target: ${edgeType.target.multiple ? "multiple" : "single"}, ${
                  edgeType.target.required ? "required" : "optional"
                }`,
              );
              console.log(`Data Schema:`);
    ...
    case "inspect":
      {
        ...
        if (!node) {
          ...
          if (nodes.length === 0) {
            ...
            nodes.forEach((n) => {
              console.log(
                `  ${colors.green}${n.id}${colors.reset} (${n.type})`,
              );
            });
          }
          break;
        ...
        console.log(
          `${colors.bright}${colors.blue}Node Details${colors.reset}`,
        );
        ...
        if ("embedding" in node) {
          ...
          console.log(
            `Dimensions: ${colors.yellow}${embedding.length}${colors.reset}`,
          );
          console.log(
            `Sample values: ${colors.gray}[${
              embedding.slice(0, 5).map((v: number) => v.toFixed(4)).join(", ")
            }...]${colors.reset}`,
          );
        } else {
        ...
        const connectedEdges = graph.getEdges().filter((e) =>
        ...
        if (connectedEdges.length > 0) {
          ...
          connectedEdges.forEach((edge) => {
            ...
            const otherNodeId = edge.sourceNodeId === nodeId
              ? edge.targetNodeId
              : edge.sourceNodeId;
            const otherNode = graph.getNodes().find((n) =>
              n.id === otherNodeId
            );
            console.log(
              `  ${colors.green}${edge.id}${colors.reset} (${edge.type}) - ${direction} to ${colors.cyan}${otherNodeId}${colors.reset}${
        ...
        if (graph.softLinks && graph.softLinks.length > 0) {
          const relatedSoftLinks = graph.softLinks.filter((link) =>
          ...
          if (relatedSoftLinks.length > 0) {
            ...
            relatedSoftLinks.forEach((link) => {
              ...
              console.log(
                `  ${colors.magenta}${link.reason}${colors.reset} to ${colors.cyan}${otherNodeId}${colors.reset}${
                  otherNode ? ` (${otherNode.type})` : ""
                }${link.score ? ` - score: ${link.score.toFixed(3)}` : ""}${
    ...
    case "status":
      {
        console.log(
          `${colors.bright}${colors.blue}TurtleDB Status${colors.reset}`,
        );
        console.log(
          `Current Graph: ${colors.green}${currentGraph}${colors.reset}`,
        );
        console.log(
          `Embeddings: ${
            USE_MOCK_EMBED || !useRealEmbeddings
        ...
        const nodesWithEmbeddings =
          nodes.filter((n) => "embedding" in n).length;
        console.log(
          `  Nodes with embeddings: ${colors.yellow}${nodesWithEmbeddings}${colors.reset}/${nodes.length}`,
        );

        // Show soft links if available
        if (graph.softLinks && graph.softLinks.length > 0) {
          console.log(
            `  Soft links: ${colors.yellow}${graph.softLinks.length}${colors.reset}`,
          );
        }
    ...
    case "list":
      if (a[0] === "types") {
        console.log(
          `${colors.bright}${colors.blue}Node Types:${colors.reset}`,
          colorJSON(Object.keys(schema.node_types)),
        );
        console.log(
          `${colors.bright}${colors.blue}Edge Types:${colors.reset}`,
          colorJSON(Object.keys(schema.edge_types)),
        );
        break;
      }
      const nodes = graph.getNodes();
      const edges = graph.getEdges();
      console.log(
        `${colors.bright}${colors.blue}Nodes (${nodes.length}):${colors.reset}`,
        nodes.length > 0
          ? colorJSON(nodes)
          : `${colors.gray}(empty)${colors.reset}`,
      );
      console.log(
        `${colors.bright}${colors.blue}Edges (${edges.length}):${colors.reset}`,
        edges.length > 0
          ? colorJSON(edges)
          : `${colors.gray}(empty)${colors.reset}`,
      );
    ...
    case "open":
      {
        const name = a[0];
        if (!name) return console.log("open <name>");
        await saveGraph(currentGraph, graph);
    ...
    case "new":
      {
        const name = a[0];
        if (!name) return console.log("new <name>");
        await saveGraph(currentGraph, graph);
