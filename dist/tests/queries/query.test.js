...
import { PrologEngine } from '../../core/prolog.ts';
...
Deno.test('PrologEngine loads facts from example-data.json', async () => {
    ...
    // Check a sample node
    const alice = facts.find((f) => f.id === 'm1');
    if (!alice)
    ...
    // Check a sample edge
    const wroteEdge = facts.find((f) => f.id === 'e1');
    if (!wroteEdge)
        throw new Error('wroteEdge not found');
...
Deno.test('Query all active members', async () => {
    ...
    const results = engine
        .getFacts()
        .filter((f) => f.type === 'member' && f.data.isActive);
...
Deno.test('Query all posts that are published', async () => {
    ...
    const results = engine
        .getFacts()
        .filter((f) => f.type === 'post' && f.data.published);
...
Deno.test('Query all authors', async () => {
    const engine = new PrologEngine();
    await engine.loadFactsFromJSON(getExampleDataPath());
    const results = engine.getFacts().filter((f) => f.type === 'author');
    console.log('Authors:', results);
    assertEquals(results.length, 2);
    assertEquals(results.map((a) => a.data.firstName).sort(), [
        'Carol',
        'Dave',
    ]);
});
...
Deno.test('Query all authors who wrote a published publication', async () => {
    ...
    const wroteEdges = engine.getFacts().filter((f) => f.type === 'wrote');
    ...
    const authors = wroteEdges
        ...
        const pub = engine
            .getFacts()
            .find((f) => f.id === e.targetNodeId &&
            f.type === 'publication' &&
            f.data.published);
        if (pub) {
            return engine
                .getFacts()
                .find((f) => f.id === e.sourceNodeId && f.type === 'author');
        }
        return null;
    })
