import { assert, assertEquals, assertExists, } from "https://deno.land/std@0.140.0/testing/asserts.ts";
...
if (shouldRun) {
    try {
        ...
        console.warn("[embedding-search.test] Skipping real-embedding tests – model unavailable:", e.message);
        shouldRun = false;
    }
}
if (shouldRun) {
    ...
    Deno.test("EmbeddingIndex - JSON stringification works well", async () => {
        ...
        assertEquals(techResults[0].data.name, "Bob");
        ...
        assertEquals(healthResults[0].data.name, "Alice");
    });
    Deno.test("EmbeddingIndex - Node data similarity search", async () => {
        ...
        const nodes = [
            ...
            },
            {
                ...
                data: {
                    ...
                    content: "This post covers the basics of neural networks and deep learning...",
        ...
        const aiQuery = await getEmbedding("artificial intelligence machine learning neural networks");
        ...
        const resultTypes = aiResults.map((r) => r.type);
        ...
        const doctorResult = medResults.find((r) => r.data.bio?.includes("Medical doctor"));
        assertExists(doctorResult);
    });
}
else {
    Deno.test("EmbeddingIndex - Skipped (no real embeddings)", () => {
        console.log("Embedding search tests skipped. Run with RUN_REAL_EMBED=1 to enable.");
    });
}
//# sourceMappingURL=embedding-search.test.js.map