...
if (shouldRun) {
  try {
    ...
    console.warn(
      "[embeddings.integration] Skipping real-embedding tests – model unavailable:",
      e.message,
    );
    shouldRun = false;
  }
...
if (shouldRun) {
  ...
  Deno.test("Embeddings reflect semantic similarity", async () => {
    ...
    assert(
      simScore > diffScore,
      `Expected similarity score (${simScore}) to be greater than diff score (${diffScore})`,
    );
  });
  ...
  Deno.test("GraphCore.createNode embeds real vector", async () => {
    ...
    assert((node as any).embedding.length > 0);
  });
}
