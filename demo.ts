...
async function demo() {
  ...
  console.log(
    `📊 Available node types: ${Object.keys(schema.node_types).join(", ")}`,
  );
  console.log(
    `📊 Available edge types: ${Object.keys(schema.edge_types).join(", ")}\n`,
  );
  ...
  console.log(
    `🔗 AI vs Medical similarity: ${
      cosineSimilarity(embedding1, embedding2).toFixed(4)
    }`,
  );
  console.log(
    `🔗 AI vs Programming similarity: ${
      cosineSimilarity(embedding1, embedding3).toFixed(4)
    }`,
  );
  console.log(
    `🔗 Medical vs Programming similarity: ${
      cosineSimilarity(embedding2, embedding3).toFixed(4)
    }\n`,
  );
  ...
  console.log(
    `✅ Created author: ${colors.green}${aliceAuthor.id}${colors.reset}`,
  );
  ...
  console.log(
    `✅ Created author: ${colors.green}${bobAuthor.id}${colors.reset}`,
  );

  const post1 = await graph.createNode("post", {
    title: "Introduction to Neural Networks",
    content:
      "This comprehensive guide covers the fundamentals of neural networks, deep learning architectures, and practical applications in modern AI systems.",
    published: true,
  });
  ...
  const publication1 = await graph.createNode("publication", {
    ...
    content:
      "Recent breakthroughs in cardiovascular medicine, including new surgical techniques and diagnostic tools for heart disease prevention.",
    published: true,
  });
  console.log(
    `✅ Created publication: ${colors.green}${publication1.id}${colors.reset}\n`,
  );

  // Create edges
  console.log(`${colors.bright}4. Creating Relationships${colors.reset}`);

  const wroteEdge1 = await graph.createEdge(
    "wrote",
    bobAuthor.id,
    publication1.id,
    {},
  ...
  console.log(
    `✅ Created edge: ${colors.green}${bobAuthor.id}${colors.reset} wrote ${colors.green}${publication1.id}${colors.reset}\n`,
  );
  ...
  for (const node of nodes) {
    if ("embedding" in node && node.embedding) {
      const similarity = cosineSimilarity(aiQuery, node.embedding as number[]);
  ...
  aiResults.slice(0, 3).forEach((result, i) => {
    ...
    console.log(
      `   ${
        i + 1
      }. ${colors.green}${node.id}${colors.reset} (${node.type}) - similarity: ${
    ...
    if (node.type === "member" || node.type === "author") {
      ...
      console.log(
        `      📝 ${node.data.title} - ${node.data.content?.slice(0, 50)}...`,
      );
    }
  });
  console.log();
  ...
  for (const node of nodes) {
    if ("embedding" in node && node.embedding) {
      const similarity = cosineSimilarity(medQuery, node.embedding as number[]);
  ...
  console.log(colorJSON({
    ...
    embeddingLength: "embedding" in alice
      ? (alice.embedding as number[]).length
      : 0,
  }));
  console.log();
  ...
  const aliceEdges = edges.filter((e) =>
    e.sourceNodeId === alice.id || e.targetNodeId === alice.id
  );
  console.log(
    `🔗 ${colors.green}${alice.id}${colors.reset} has ${aliceEdges.length} connection(s):`,
  );
  ...
  aliceEdges.forEach((edge) => {
    ...
    console.log(
      `   ${direction} ${colors.blue}${edge.type}${colors.reset} ${colors.green}${otherNodeId}${colors.reset}`,
    );
  ...
  console.log(
    `${colors.gray}TurtleDB successfully demonstrated:${colors.reset}`,
  );
  console.log(
    `${colors.gray}  ✓ Schema-based node/edge creation${colors.reset}`,
  );
  console.log(
    `${colors.gray}  ✓ Deterministic embedding generation${colors.reset}`,
  );
